...
@section('content')
<div class="container">
    <h1 class="mb-4 text-xl font-bold">Tambah Anggota</h1>
    @if ($errors->any())
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="jabatan" name="jabatan" required onchange="toggleBidangRequirement()">
                ...
                @foreach ($jabatanOptions as $value => $label)
                    <option value="{{ $value }}" @selected(old('jabatan') === $value)>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="bidang_id" name="bidang_id">
                ...
                @foreach ($bidangs as $bidang)
                    <option value="{{ $bidang['id'] }}" @selected(old('bidang_id') == $bidang['id'])>{{ $bidang['nama'] }}</option>
                @endforeach
            </select>
            <small class="text-muted">Ketua Umum, Sekretaris Umum, dan Bendahara Umum boleh tanpa bidang.</small>
        </div>
        <div class="mb-3">
